...
output_dir = '/home/sh38/Mask_RCNN/datasets/cell_cycle_down2/'
...
for image_indx in range(len(image_ids)):
	...
	pha_img = io.imread(phase_img_folder+pha_file_name); pha_img = pha_img[::2,::2]
	map = io.imread(combined_masks+mask_file_name); map = map[::2,::2]
	if image_indx%10==0:
	...
	for cls_indx in range(1,len(uni_values)):
		...
		for i in range(len(obj_list)):
			...
			if np.sum(mask)>128:
				mask_list.append(mask) ## add to the mask list if the mask size is larger than 128 (filtering)
	...
	io.imsave(data_folder+'/GT/{}.png'.format(new_image_id),np.uint8(recon_mask))
	io.imsave(data_folder+'/original_GT/{}.png'.format(new_image_id),map)
	for i in range(len(mask_list)):
		io.imsave(data_folder+'/masks/mask_{:04d}.png'.format(i),np.uint8(mask_list[i]*map))