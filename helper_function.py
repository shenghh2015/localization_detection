...
### for proGAN and anomaly detection based optimization
def plot_optim_loss(file_name, loss_list, interval):
	from matplotlib.backends.backend_agg import FigureCanvasAgg
	from matplotlib.figure import Figure
	fig_size = (8,6)
	fig = Figure(figsize=fig_size)
	...
	title = os.path.basename(os.path.dirname(file_name))
	ax.set_title(title)
	ax.set_xlabel('Interations/{}'.format(int(interval)))
	ax.set_ylabel('Optim loss')
# 	ax.legend(['Detection'])
	ax.set_xlim([0,len(loss_list)])
	canvas = FigureCanvasAgg(fig)
	canvas.print_figure(file_name, dpi=100)

def plot_hist_list(file_name, stat_list, legend_list, color_list, range_tuple, xlabel = 'Error', bins = 100):
	import matplotlib.pyplot as plt
	from matplotlib.backends.backend_agg import FigureCanvasAgg
	from matplotlib.figure import Figure
	kwargs = dict(alpha=0.5, bins=bins, density= False, range = range_tuple, stacked=True)
	fig_size = (8,6)
	fig = Figure(figsize=fig_size)
	ax = fig.add_subplot(111)
	for i in range(len(stat_list)):
